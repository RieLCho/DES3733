...
# Step 8: Train with AdversarialTrainer Madry PGD for accuracy comparision
...
accuracy = np.sum(np.argmax(classifier_trained.predict(x_test_adv), axis=1) == np.argmax(y_test, axis=1)) / len(y_test)
